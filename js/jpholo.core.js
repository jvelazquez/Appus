...
function panelMenu(divId) {
	...
	panel.append('<li data-role="list-divider"><p class="panelTextDivider">Universidad del Sur</p></li>');
	panel.append('<li data-icon="false"><a class="panelText" href="#indexPage"><img src="./images/icons/ic_action_home.png" class="ui-li-icon largerIcon">Inicio</a></li>');
	//panel.append('<li data-role="list-divider"><p class="panelTextDivider">Other pages</p></li>');
	panel.append('<li data-icon="false"><a class="panelText" href="#calificacion"><img src="./images/icons/ic_action_info.png" class="ui-li-icon largerIcon">Calificacion</a></li>');
	panel.append('<li data-icon="false"><a class="panelText" href="#horario"><img src="./images/icons/ic_action_info.png" class="ui-li-icon largerIcon">Horario</a></li>');
...
$(document).on('pagebeforeshow', '#indexPage', function (event, data) {
	...
	var formData = window.localStorage["iduser"];
   $.ajax({
	   
         	type: 'POST',
            dataType: 'json',
            url:'servidor/ajax/datosalumnos.php',
            data:{ idalum:formData}
	   
        }).done(function(respuesta){
        	$("#alumn_mat").val(respuesta.matricula);
			$("#alumn_nom").val(respuesta.nombre);
     		$("#alumn_nivelac").val(respuesta.nivelcad);
			$("#alumn_hor").val(respuesta.asistencia);
			$("#alumn_grupo").val(respuesta.grupo);
 			$("#alumn_turn").val(respuesta.turno);
    });
	//alert('alert1');
...
// #Calificacion
$(document).on('pagebeforeshow', '#calificacion', function (event, data) {
        	...
        	if(data.length > 0){
                  $.each(data, function(i,item){
					  html += '<tr>'
							...
							html += '<td>'+item.p4+'</td>'
							html += '<td>'+item.i4+'</td>'
							html += '<td>'+item.p_final+'</td>'
							html += '<td>'+item.i_final+'</td>'
                        html += '</tr>';
				    });
              } 
...
$(document).on('pagecreate', '#calificacion', function () {
	initPageVarsOnCreate('Other');
});

// #Horario
$(document).on('pagebeforeshow', '#horario', function (event, data) {

	
	startBeforeShowVars(data);
	initPageVarsOnShow('horario');
...
var formData = window.localStorage["iduser"];
   $.ajax({
	   ...
	   if(html == '') html = '<tr><td colspan="6">No se encontraron registros..</td></tr>'
                // añadimos  a nuestra tabla todos los datos encontrados mediante la funcion html
                $("#horario_alumno tbody").html(html);  
...
$(document).on('pagecreate', '#horario', function () {
	initPageVarsOnCreate('Other');
});
